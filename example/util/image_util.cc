...
static inline double getTimestampFromPath(const std::string &path) {
    std::string name = path.substr(path.find_last_of("/")+1);
    double ts = std::stod(name);
    while (ts>2000000000) { //ts as mili or nanoseconds
        ts /= 10;
    }
    return ts;
}
...
std::vector<image_sequence::frame> image_sequence::get_frames() const {
    ...
    for (unsigned int i = 0; i < img_file_paths_.size(); ++i) {
        frames.emplace_back(frame{img_file_paths_.at(i), getTimestampFromPath(img_file_paths_.at(i))});
    }
    return frames;
}
