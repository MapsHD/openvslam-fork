...
RUN set -x && \
  ...
  mkdir -p .cache && cd .cache && unzip /tmp/patch.zip && cd .. && \
  mkdir -p build && \
  cd build && \
  cmake \
    -DCMAKE_BUILD_TYPE=Release \
    -DOPENCV_EXTRA_MODULES_PATH=/tmp/opencv_contrib-${OPENCV_CONTRIB_VERSION}/modules/ \
    -DCMAKE_INSTALL_PREFIX=${CMAKE_INSTALL_PREFIX} \
    -DBUILD_DOCS=OFF \
    -DBUILD_EXAMPLES=OFF \
    -DBUILD_JASPER=OFF \
...
RUN apt-get install -y liblas-dev liblas-c-dev freeglut3-dev libpcl-dev
RUN apt-get install -y vim htop screen
