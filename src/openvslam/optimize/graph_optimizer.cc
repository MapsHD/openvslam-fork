...
void graph_optimizer::optimize() const {
	...
	for (auto keyfrm : all_keyfrms) {
		if (keyfrm->will_be_erased()) {
			std::cout << "keyfrm->will_be_erased() !!!" << std::endl;
			continue;
		}
		auto keyfrm_vtx = new internal::sim3::shot_vertex();
	...
	for (auto keyfrm : all_keyfrms) {
		...
		std::cout << "id: " << id1 << " " << Sim3s_cw.at(id1) << std::endl;
		...
		auto parent_node = keyfrm->graph_node_->get_spanning_parent();
		if (parent_node) {
			std::cout << "found parrent node" << std::endl;
			const auto id2 = parent_node->id_;

			// Avoid duplication
			if (id1 <= id2) {
				std::cout << "avoid duplications" << std::endl;
				continue;
			}
			...
			// Add a constraint edge
			insert_edge(id1, id2, Sim3s_cw.at(id2) * Sim3s_cw.at(id1).inverse());
		}
	...
	rp.translation().y() = 0;
	rp.translation().z() = 0;
	insert_edge(0, max_keyfrm_id, rp);
...

	{
		...
		for (auto lm : all_lms) {
			...
			const auto id = lm->get_ref_keyframe()->id_;
